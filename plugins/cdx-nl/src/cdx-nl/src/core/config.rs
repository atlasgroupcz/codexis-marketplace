...
pub(crate) const CODEXIS_PLUGIN_NL_API_URL_ENV: &str = "CODEXIS_PLUGIN_NL_API_URL";
...
impl Config {
    pub(crate) fn load() -> Result<Self, CliError> {
        let base_url = read_env(CODEXIS_PLUGIN_NL_API_URL_ENV);
        ...
        match base_url {
            ...
            None => Err(CliError::MissingConfig(CODEXIS_PLUGIN_NL_API_URL_ENV)),
        }
    }
}
