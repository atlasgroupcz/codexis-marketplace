...
|---|---|---|---|---|
| `ares search <query>` | "Najdi firmu podle názvu." | `POST /ekonomicke-subjekty/vyhledat` | Uživatel zadá název nebo část názvu. | Lokálně seřazené `kandidati`: `nazev`, `ico`, `sidlo`, `pravniForma`, `primarniZdroj`, `stavRegistraci`. |
...
`ares search` může načíst širší sadu výsledků, než kolik vrátí podle `--limit`, a lokálně ji seřadit podle relevance. Důvodem je zabránit tomu, aby přesná shoda obchodní firmy zůstala skrytá za dceřinými nebo podobně pojmenovanými subjekty.
