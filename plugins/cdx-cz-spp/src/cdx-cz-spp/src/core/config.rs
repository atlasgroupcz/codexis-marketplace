...
pub(crate) const CODEXIS_PUBLIC_CZ_SPP_API_URL_ENV: &str = "CODEXIS_PUBLIC_CZ_SPP_API_URL";
...
impl Config {
    pub(crate) fn load() -> Result<Self, CliError> {
        ...
        let base_url = resolve_config_value(CODEXIS_PUBLIC_CZ_SPP_API_URL_ENV, env_file.as_ref());
        ...
        match base_url {
            ...
            None => Err(CliError::MissingConfig(CODEXIS_PUBLIC_CZ_SPP_API_URL_ENV)),
        }
    }
}
...
mod tests {
    ...
    fn env_parser_supports_plain_and_exported_values() {
        let parsed = parse_env_file(
            r#"
CODEXIS_PUBLIC_CZ_SPP_API_URL=https://search.example.com/api
...
"#,
        ...
        assert_eq!(
            parsed.get(CODEXIS_PUBLIC_CZ_SPP_API_URL_ENV),
    ...
    fn env_parser_unescapes_double_quoted_values() {
        let parsed = parse_env_file(r#"CODEXIS_PUBLIC_CZ_SPP_API_URL="https://example.com/\"api\"""#);
        assert_eq!(
            parsed.get(CODEXIS_PUBLIC_CZ_SPP_API_URL_ENV),
    ...
    fn resolve_config_value_prefers_process_env_and_rejects_empty_values() {
        ...
        let value =
            resolve_config_value_for_test(Some("   "), CODEXIS_PUBLIC_CZ_SPP_API_URL_ENV, Some(&env_file));
        assert_eq!(
            value,
    ...
    fn resolve_config_value_uses_env_file_when_process_env_is_missing() {
        ...
        let value = resolve_config_value_for_test(None, CODEXIS_PUBLIC_CZ_SPP_API_URL_ENV, Some(&env_file));
