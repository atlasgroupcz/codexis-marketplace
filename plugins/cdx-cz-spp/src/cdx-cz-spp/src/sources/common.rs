...
mod tests {
    ...
    use super::*;
    use crate::sources::czsb::SearchCzsbArgs;

    #[test]
    fn json_payload_overrides_matching_flag_values() {
        let args = SearchCzsbArgs {
            base: SearchBaseArgs {
            ...
            ..SearchCzsbArgs::default()
        };

        let payload = args.build_payload("CZSB").unwrap();
        assert_eq!(
            payload,
    ...
    fn search_payload_can_be_built_from_flags_only() {
        let args = SearchCzsbArgs {
            base: SearchBaseArgs {
                query: Some("vyhláška".to_string()),
                limit: Some(5),
                ..SearchBaseArgs::default()
            },
            publikujici: Some("Praha".to_string()),
            ..SearchCzsbArgs::default()
        };

        let payload = args.build_payload("CZSB").unwrap();
        let parsed: Value = serde_json::from_str(&payload).unwrap();
        assert_eq!(parsed["query"], "vyhláška");
        assert_eq!(parsed["limit"], 5);
        assert_eq!(parsed["offset"], 0);
        assert_eq!(parsed["publikujici"], "Praha");
    }
