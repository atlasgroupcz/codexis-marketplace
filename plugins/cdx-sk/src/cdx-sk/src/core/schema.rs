...
fn render_generic_source_schema(kind: ResourceSchemaKind) -> Result<String, CliError> {
    ...
    let subcommands = available_sources
        .iter()
        .map(|source| format!("`cdx-sk schema {} {}`", resource_endpoint_name(kind), source.source_code()))
        .collect::<Vec<_>>()
        .join(", ");
...
fn load_source_aware_schema_bundle(
    ...
    match (kind, source) {
        (ResourceSchemaKind::Meta, SchemaSource::Skez) => {
            Ok(include_str!("../../schemas/resource/meta-skez.bundle.json"))
        }
        (ResourceSchemaKind::Meta, SchemaSource::Skvs) => {
            Ok(include_str!("../../schemas/resource/meta-skvs.bundle.json"))
        }
        (ResourceSchemaKind::Meta, SchemaSource::Sknus) => {
            Ok(include_str!("../../schemas/resource/meta-sknus.bundle.json"))
        }
        (ResourceSchemaKind::Parts, SchemaSource::Skez) => {
            Ok(include_str!("../../schemas/resource/parts-skez.bundle.json"))
        }
        (ResourceSchemaKind::Parts, SchemaSource::Skvs) => {
            Ok(include_str!("../../schemas/resource/parts-skvs.bundle.json"))
        }
        (ResourceSchemaKind::Parts, SchemaSource::Sknus) => {
            Ok(include_str!("../../schemas/resource/parts-sknus.bundle.json"))
        }
        (ResourceSchemaKind::Versions, SchemaSource::Skez) => Ok(include_str!(
            "../../schemas/resource/versions-skez.bundle.json"
        )),
        (ResourceSchemaKind::Versions, SchemaSource::Skvs) => {
            Err(CliError::InvalidStoredSchema(
                "the /versions endpoint is not available for SKVS documents".to_string(),
            ))
        }
        (ResourceSchemaKind::Versions, SchemaSource::Sknus) => Ok(include_str!(
            "../../schemas/resource/versions-sknus.bundle.json"
        )),
        (ResourceSchemaKind::Toc, SchemaSource::Skez) => {
            Ok(include_str!("../../schemas/resource/toc-skez.bundle.json"))
        }
        (ResourceSchemaKind::Toc, SchemaSource::Skvs) => Err(CliError::InvalidStoredSchema(
            "the /toc endpoint is not available for SKVS documents".to_string(),
        )),
        (ResourceSchemaKind::Toc, SchemaSource::Sknus) => Err(CliError::InvalidStoredSchema(
            "the /toc endpoint is not available for SKNUS documents".to_string(),
        )),
        (ResourceSchemaKind::Related, SchemaSource::Skez) => Ok(include_str!(
            "../../schemas/resource/related-skez.bundle.json"
        )),
        (ResourceSchemaKind::Related, SchemaSource::Skvs) => Ok(include_str!(
            "../../schemas/resource/related-skvs.bundle.json"
        )),
        (ResourceSchemaKind::Related, SchemaSource::Sknus) => Ok(include_str!(
            "../../schemas/resource/related-sknus.bundle.json"
        )),
        _ => Err(CliError::InvalidStoredSchema(format!(
            "/{} is not source-aware",
            resource_endpoint_name(kind)
...
fn resource_output_description(kind: ResourceSchemaKind) -> &'static str {
    match kind {
        ResourceSchemaKind::Meta => "Metadata for one document or Slovak law.",
...
fn resource_cdx_patterns(kind: ResourceSchemaKind) -> Vec<String> {
    ...
    let mut patterns = vec![format!("cdx-sk://doc/<DOC_ID>/{endpoint}")];
    // law patterns only for SKEZ-relevant endpoints
    if matches!(
        kind,
        ResourceSchemaKind::Meta
            | ResourceSchemaKind::Text
            | ResourceSchemaKind::Toc
            | ResourceSchemaKind::Parts
            | ResourceSchemaKind::Versions
            | ResourceSchemaKind::Related
            | ResourceSchemaKind::RelatedCounts
    ) {
        patterns.push(format!("cdx-sk://law/SK/<NUM>/<YEAR>/{endpoint}"));
    }
...
fn source_specific_cdx_patterns(
    ...
    let mut patterns = vec![format!("cdx-sk://doc/<DOC_ID>/{endpoint}")];
    if matches!(source, SchemaSource::Skez) {
        patterns.push(format!("cdx-sk://law/SK/<NUM>/<YEAR>/{endpoint}"));
    }
...
fn render_resource_intro(
    ...
    lines.push("Accepted cdx-sk:// patterns:".to_string());
...
impl ResourceSchemaKind {
    ...
    fn available_sources(self) -> Vec<SchemaSource> {
        match self {
            Self::Meta | Self::Parts | Self::Related => {
                vec![SchemaSource::Skez, SchemaSource::Skvs, SchemaSource::Sknus]
            }
            Self::Versions => vec![SchemaSource::Skez, SchemaSource::Sknus],
            Self::Toc => vec![SchemaSource::Skez],
            _ => vec![],
        }
    }
}
...
mod tests {
    ...
    fn generic_meta_schema_points_to_typed_subcommands() {
        ...
        assert!(intro.contains("cdx-sk://doc/<DOC_ID>/meta"));
        assert!(intro.contains("cdx-sk schema meta SKEZ"));
    ...
    fn typed_meta_schema_renders_source_specific_bundle() {
        let output =
            render_resource_schema(ResourceSchemaKind::Meta, Some(SchemaSource::Skez)).unwrap();
        let (intro, schema) = split_rendered_schema_document(&output).unwrap();

        assert!(intro.contains("Endpoint: /meta"));
        assert!(intro.contains("SKEZ"));
        assert!(intro.contains("cdx-sk://doc/<DOC_ID>/meta"));
        assert!(intro.contains("cdx-sk://law/SK/<NUM>/<YEAR>/meta"));
    ...
    #[test]
    fn toc_skez_renders_successfully() {
        let output =
            render_resource_schema(ResourceSchemaKind::Toc, Some(SchemaSource::Skez)).unwrap();
    ...
    #[test]
    fn toc_skvs_returns_error() {
        let result =
            render_resource_schema(ResourceSchemaKind::Toc, Some(SchemaSource::Skvs));
        assert!(result.is_err());
    }

    #[test]
    fn toc_sknus_returns_error() {
        let result =
            render_resource_schema(ResourceSchemaKind::Toc, Some(SchemaSource::Sknus));
        assert!(result.is_err());
    }

    #[test]
    fn versions_skvs_returns_error() {
        let result =
            render_resource_schema(ResourceSchemaKind::Versions, Some(SchemaSource::Skvs));
        assert!(result.is_err());
    }
    ...
    fn generic_parts_schema_lists_subcommands() {
        ...
        assert!(output.contains("cdx-sk schema parts SKEZ"));
        assert!(output.contains("cdx-sk schema parts SKVS"));
        assert!(output.contains("cdx-sk schema parts SKNUS"));
    }

    #[test]
    fn skvs_meta_does_not_include_law_pattern() {
        let output =
            render_resource_schema(ResourceSchemaKind::Meta, Some(SchemaSource::Skvs)).unwrap();
        let (intro, _) = split_rendered_schema_document(&output).unwrap();
