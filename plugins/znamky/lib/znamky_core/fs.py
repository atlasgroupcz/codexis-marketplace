"""Filesystem utilities shared across znamky_core (watch-state I/O)."""
...
def atomic_replace(path, write_fn, mode=None):
    ...
    `mode` — if given, the tmp file is chmod'ed before the rename.
    """
...
def atomic_write_bytes(path, payload: bytes):
    """Atomically write raw bytes (e.g. a stored logo) to `path`."""
    d = os.path.dirname(path) or "."
    os.makedirs(d, exist_ok=True)
    fd, tmp = tempfile.mkstemp(prefix=os.path.basename(path) + ".", dir=d)
    try:
