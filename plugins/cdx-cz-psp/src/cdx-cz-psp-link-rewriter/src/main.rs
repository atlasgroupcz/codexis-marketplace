...
fn main() {
    let base_url = match std::env::var("CODEXIS_PLUGIN_CZ_PSP_API_URL") {
        Ok(value) if !value.trim().is_empty() => value.trim().to_string(),
        _ => {
            eprintln!("CODEXIS_PLUGIN_CZ_PSP_API_URL must be set (e.g., export CODEXIS_PLUGIN_CZ_PSP_API_URL=\"https://search.example.com/api\")");
            std::process::exit(2);
        }
    };
