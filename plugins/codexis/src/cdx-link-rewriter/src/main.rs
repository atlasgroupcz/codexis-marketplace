...
fn main() {
    let base_url = match env::var("CODEXIS_PLUGIN_BASE_URL") {
        Ok(value) if !value.trim().is_empty() => value.trim().trim_end_matches('/').to_string(),
        _ => {
            eprintln!("CODEXIS_PLUGIN_BASE_URL must be set (e.g., export CODEXIS_PLUGIN_BASE_URL=\"https://next.codexis.cz\")");
            std::process::exit(2);
        }
    };
