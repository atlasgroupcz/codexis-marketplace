...
mod tests {
    ...
    use super::*;
    use crate::sources::atbr::SearchAtbrArgs;
    use crate::sources::atjd::SearchAtjdArgs;

    #[test]
    fn json_payload_overrides_matching_flag_values() {
        let args = SearchAtjdArgs {
            base: SearchBaseArgs {
            ...
            ..SearchAtjdArgs::default()
        };

        let payload = args.build_payload("ATJD").unwrap();
        assert_eq!(
            payload,
    ...
    fn search_payload_can_be_built_from_flags_only() {
        let args = SearchAtbrArgs {
            base: SearchBaseArgs {
                query: Some("Verordnung".to_string()),
                limit: Some(5),
                ..SearchBaseArgs::default()
            },
            document_type: Some("Verordnung".to_string()),
            ..SearchAtbrArgs::default()
        };

        let payload = args.build_payload("ATBR").unwrap();
        let parsed: Value = serde_json::from_str(&payload).unwrap();
        assert_eq!(parsed["query"], "Verordnung");
        assert_eq!(parsed["limit"], 5);
        assert_eq!(parsed["offset"], 0);
        assert_eq!(parsed["documentType"], "Verordnung");
    }
