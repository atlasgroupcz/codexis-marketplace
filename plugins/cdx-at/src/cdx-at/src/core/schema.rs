...
fn render_generic_source_schema(kind: ResourceSchemaKind) -> Result<String, CliError> {
    ...
    let subcommands = available_sources
        .iter()
        .map(|source| format!("`cdx-at schema {} {}`", resource_endpoint_name(kind), source.source_code()))
        .collect::<Vec<_>>()
        .join(", ");
...
fn load_source_aware_schema_bundle(
    ...
    match (kind, source) {
        (ResourceSchemaKind::Meta, SchemaSource::Atbr) => {
            Ok(include_str!("../../schemas/resource/meta-atbr.bundle.json"))
        }
        (ResourceSchemaKind::Meta, SchemaSource::Atjd) => {
            Ok(include_str!("../../schemas/resource/meta-atjd.bundle.json"))
        }
        (ResourceSchemaKind::Meta, SchemaSource::Atlr) => {
            Ok(include_str!("../../schemas/resource/meta-atlr.bundle.json"))
        }
        (ResourceSchemaKind::Meta, SchemaSource::Atso) => {
            Ok(include_str!("../../schemas/resource/meta-atso.bundle.json"))
        }
        (ResourceSchemaKind::Meta, SchemaSource::Athi) => {
            Ok(include_str!("../../schemas/resource/meta-athi.bundle.json"))
        }
        _ => Err(CliError::InvalidStoredSchema(format!(
            "/{} is not source-aware",
            resource_endpoint_name(kind)
...
fn resource_cdx_patterns(kind: ResourceSchemaKind) -> Vec<String> {
    ...
    vec![format!("cdx-at://doc/<DOC_ID>/{endpoint}")]
}
...
fn render_resource_intro(
    ...
    lines.push("Accepted cdx-at:// patterns:".to_string());
...
mod tests {
    ...
    fn generic_meta_schema_points_to_typed_subcommands() {
        ...
        assert!(intro.contains("cdx-at://doc/<DOC_ID>/meta"));
        assert!(intro.contains("cdx-at schema meta ATBR"));
        assert!(intro.contains("cdx-at schema meta ATHI"));
    ...
    fn typed_meta_schema_renders_source_specific_bundle() {
        let output =
            render_resource_schema(ResourceSchemaKind::Meta, Some(SchemaSource::Atbr)).unwrap();
        let (intro, schema) = split_rendered_schema_document(&output).unwrap();

        assert!(intro.contains("Endpoint: /meta"));
        assert!(intro.contains("ATBR"));
        assert!(intro.contains("cdx-at://doc/<DOC_ID>/meta"));
    ...
    #[test]
    fn all_five_meta_sources_render_successfully() {
        for source in [
            SchemaSource::Atbr,
            SchemaSource::Atjd,
            SchemaSource::Atlr,
    ...
    #[test]
    fn generic_meta_schema_lists_all_source_subcommands() {
        let output = render_resource_schema(ResourceSchemaKind::Meta, None).unwrap();
        assert!(output.contains("cdx-at schema meta ATBR"));
        assert!(output.contains("cdx-at schema meta ATJD"));
        assert!(output.contains("cdx-at schema meta ATLR"));
        assert!(output.contains("cdx-at schema meta ATSO"));
        assert!(output.contains("cdx-at schema meta ATHI"));
    }

    #[test]
    fn meta_does_not_include_law_pattern() {
        let output =
            render_resource_schema(ResourceSchemaKind::Meta, Some(SchemaSource::Atbr)).unwrap();
        let (intro, _) = split_rendered_schema_document(&output).unwrap();
