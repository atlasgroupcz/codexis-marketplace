...
fn main() {
    ...
    let output = rewrite_cdx_at_links(&input, &base_url, signing_secret.as_deref());
    print!("{output}");
}

fn rewrite_cdx_at_links(html: &str, base_url: &str, signing_secret: Option<&str>) -> String {
    ...
    while let Some(href_start) = remaining.find(needle) {
        ...
        match resolve_cdx_at_path(path, base, signing_secret) {
            Some(resolved) => {
...
fn resolve_cdx_at_path(path: &str, base: &str, signing_secret: Option<&str>) -> Option<String> {
...
mod tests {
    ...
    fn rewrites_atbr_attachment_link() {
        ...
        assert_eq!(rewrite_cdx_at_links(input, BASE, None), expected);
    }
    ...
    fn ignores_cdx_sk_scheme() {
        ...
        assert_eq!(rewrite_cdx_at_links(input, BASE, None), input);
    }
    ...
    fn handles_empty_input() {
        assert_eq!(rewrite_cdx_at_links("", BASE, None), "");
    }
    ...
    fn base_url_trailing_slash_stripped() {
        ...
        let expected = r#"<a href="https://search.example.com/api/AT/bundesrecht/doc/ATBR1/attachment/a.pdf" target="_blank">Doc</a>"#;
        assert_eq!(rewrite_cdx_at_links(input, "https://search.example.com/api/", None), expected);
    }
    ...
    fn attachment_link_gets_sig_when_secret_present() {
        let input = r#"<a href="cdx-at://doc/ATBR1234/attachment/content_1.pdf">Doc</a>"#;
        let output = rewrite_cdx_at_links(input, BASE, Some(TEST_SECRET));
    ...
    fn attachment_link_no_sig_when_secret_absent() {
        let input = r#"<a href="cdx-at://doc/ATBR1234/attachment/content_1.pdf">Doc</a>"#;
        let output = rewrite_cdx_at_links(input, BASE, None);
        assert!(!output.contains("?sig="));
    }
    ...
    fn non_attachment_doc_link_no_sig() {
        let input = r#"<a href="cdx-at://doc/ATBR1234/meta">Doc</a>"#;
        let output = rewrite_cdx_at_links(input, BASE, Some(TEST_SECRET));
        assert!(!output.contains("?sig="));
    }
