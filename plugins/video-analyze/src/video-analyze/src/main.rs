...
fn main() {
    ...
    if args.len() != 2 || args.iter().any(|arg| arg == "-h" || arg == "--help") {
        ...
        eprintln!();
        eprintln!("  source  YouTube URL or file path to video");
        eprintln!("  query   What to analyze in the video");
    ...
    let encoded_source = percent_encode(source.as_bytes());
    let encoded_query = percent_encode(query.as_bytes());

    let url = format!(
        "{}/rest/video/analyze?source={}&query={}",
        daemon_url, encoded_source, encoded_query
    );
